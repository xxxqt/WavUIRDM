...
class AlignedDataset_all(BaseDataset):
    ...
    def __init__(self, opt, image_size, augment_flip=True, equalizeHist=True, crop_patch=True, generation=False, task=None):
        ...
        if opt.phase == 'train':
            ...
            self.dir_Asnow = os.path.join(opt.dataroot, 'Snow100K/' + opt.phase + '/Snow100K-L/synthetic') #Snow100K-S Snow100K-L
            self.dir_Bsnow = os.path.join(opt.dataroot, 'Snow100K/' + opt.phase + '/Snow100K-L/gt')
            self.dir_Arain_syn = os.path.join(opt.dataroot, 'syn_rain/' + opt.phase + '/Test100/input') #Rain100H, Rain100L, Test100, Test1200,Test2800
            self.dir_Brain_syn = os.path.join(opt.dataroot, 'syn_rain/' + opt.phase + '/Test100/target')   #Test2800 Test2800
        ...
        elif task == '4':  # mix rain/lsrw.lol/snow
        ...
        elif task == '5':    # mix rain/lsrw.lol/snow/blur
        ...
        elif task == '6':    # mix rain/lsrw.lol/snow/blur/fog
    ...
    def __getitem__(self, index):
        ...
        condition = Image.open(A_path).convert('RGB')
        gt = Image.open(B_path).convert('RGB')
        ...
        if 'LOL' in A_path or 'LSRW' in A_path:
            ...
            if self.opt.phase == 'train':
                if h < 256 or w < 256:
                    ...
                    gt = resi(gt)
        deg_type = self.get_deg_type(B_path)
        return {'adap': condition, 'gt': gt, 'A_paths': A_path, 'B_paths': B_path, "type_paths": deg_type }
    ...
    def get_deg_type(self,B_path: str) -> str:
        lower_path = B_path.lower()
        if "reside" in lower_path:
            return "fog"
        if "deblur" in lower_path:
            return "blur"
        if "lol" in lower_path or "lsrw" in lower_path:
            return "light_only"
        if "rain" in lower_path:
            return "rain"
        if "snow" in lower_path:
            return "snow"
        print(f"Warning: '{B_path}' did not match any known degradation type.")
        return "unknown"
