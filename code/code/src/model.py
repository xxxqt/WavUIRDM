...
import importlib
from  src.wavelet2D import  DWT, IWT
from pytorch_msssim import ssim
import torch

def pad_to_multiple(x, base=4, mode='reflect'):
    _, _, h, w = x.shape
    pad_h = (base - h % base) % base
    pad_w = (base - w % base) % base
    x_padded = F.pad(x, (0, pad_w, 0, pad_h), mode=mode)  # pad=(left, right, top, bottom)
...
def photo_loss(pred_x, gt_img):
    l1_loss = torch.nn.L1Loss()
    device = gt_img.device
    content_loss = l1_loss(pred_x, gt_img)
    ssim_loss = 1 - ssim(pred_x, gt_img, data_range=2.0).to(device)
    photo_loss = 1 * content_loss + 1 * ssim_loss
    return  photo_loss
...
class Unet(nn.Module):
    def __init__(
        ...
        input_channels = channels + channels * (1 if condition else 0)  + channels * (3 if wave_condition else 0)  #  XXXXXXXX
...
class UnetRes(nn.Module):
    def __init__(
        ...
        self.wave_condition = wave_condition
        self.channels = channels
        default_out_dim = channels * (1 if not learned_variance else 2)
        self.out_dim = default(out_dim, default_out_dim)
        self.random_or_learned_sinusoidal_cond = learned_sinusoidal_cond or random_fourier_features
...
class ResidualDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.clip1 = partial(torch.clamp, min=-4., max=4.)
        self.clip0 = partial(torch.clamp, min=-2., max=2.)
        self.clipno = partial(torch.clamp, min=-1., max=1.)
        self.model = model
        ...
        self.wave_cond =wave_cond
        # 初始化模型并加载预训练参数
        self.refine_model = refine_model
        self.cls_model = cls_model
        if refine_ckpt_path is not None:
            ckpt = torch.load(refine_ckpt_path)
            self.refine_model.load_state_dict(ckpt['recon'])
            self.cls_model.load_state_dict(ckpt['cls'])
        for param in self.cls_model.parameters():
            param.requires_grad = False
        self.cls_model.eval()   #  冻结分类器
        # —— Hook 机制：在 HFRM 的 encoders 模块上注册 forward hook ——
        self.hook_outputs = []
        self.hooks = []
        print("hook")
        # HFRM.encoders 是一个 ModuleList，每个阶段输出一个 feature map
        for module in self.refine_model.encoders:
    ...
    def remove_hooks(self):
        ...
        self.hooks.clear()
    def eval(self):
        # 自定义行为：先移除 hooks，再设置为 eval 模式
        self.remove_hooks()
        return super().eval()  # 保留原始行为
    ...
    def model_predictions(self, x_input,wave_input, x, t,  clip_denoised=True):
        if not self.condition:
            ...
            if self.wave_cond:
                ...
                wave_d =wave_input[1]
                wave_v =wave_input[2]
                x_in = torch.cat((x, x_input,wave_h,wave_d,wave_v), dim=1)
            else:
        ...
        maybe_clip = partial(torch.clamp, min=-4.,
                             max=4.) if clip_denoised else identity
        ...
        if self.objective == 'pred_res_noise':
            ...
            x_start = x_input - pred_res
            x_start = maybe_clip(x_start)          #         wavelet?????? *********
    ...
    def p_sample_loop(self, x_input, shape, last=True):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps,disable=self.training ):
    ...
    @torch.no_grad()
    def ddim_sample(self, x_input,wave_input, shape, last=True):
        x_input = x_input[0]
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step',disable=self.training ):
            time_cond = torch.full(
                (batch,), time, device=device, dtype=torch.long)
            preds = self.model_predictions(x_input, wave_input, img, time_cond)
    ...
    def sample(self, x_input=0, batch_size=16, last=True, task=None):
        ...
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample   #true
        if True:
            wave_input = x_input[1:]
            x_input = x_input[0]
            x_input = x_input.unsqueeze(0)
            batch_size, channels, h, w = x_input[0].shape
            size = (batch_size, channels, h, w)
        return sample_fn(x_input,wave_input, size, last=last)
    ...
    def p_losses(self, imgs, t, noise=None):  # GT + DIFF_INPUT
        if isinstance(imgs, list):  # Condition
            x_input = imgs[1]
            x_start = imgs[0]  #gt:imgs[0], cond:imgs[1]
            wave_input = imgs[2:]
        noise = default(noise, lambda: torch.randn_like(x_start))
    ...
    def estimation_loss(self, pred, gt):
        b, c, h, w = pred.shape
        dwt0 = self.dwt(pred)
        pred_LL, pred_high0 = dwt0[:b, ...], dwt0[b:, ...]
        dwt1 = self.dwt(pred_LL)
        pred_LL_LL, pred_high1 = dwt1[:b, ...], dwt1[b:, ...]
        # gt
        gt_dwt0 = self.dwt(gt)
        gt_LL, gt_high0 = gt_dwt0[:b, ...], gt_dwt0[b:, ...]
        gt_dwt1 = self.dwt(gt_LL)
        gt_LL_LL, gt_high1 = gt_dwt1[:b, ...], gt_dwt1[b:, ...]
        # =============frequency loss==================
        frequency_loss = 1 * (self.l2_loss(pred_high0, gt_high0) +
                              self.l2_loss(pred_high1, gt_high1) +
                              self.l2_loss(pred_LL, gt_LL)) + \
                         0.00005 * (self.TV_loss(pred_high0) +
                                    self.TV_loss(pred_high1) +
                                    self.TV_loss(pred_LL))
        # =============photo loss==================  #
        photo_loss_pred = photo_loss(pred, gt)
        return photo_loss_pred, frequency_loss

    def cls_loss(self, refine_in , feature, gt_labels):
        device = refine_in.device
        self.cls_model = self.cls_model.to(device)
        logits = self.cls_model(refine_in, feature)
        return F.cross_entropy(logits, gt_labels)

    @torch.no_grad()
    def restore(self,img, refine_hf = True):
        if isinstance(img, list):
        ...
        # *******wavelet input ******************
        input_img = img[0]
        input_dwt = self.dwt(input_img)
        input_LL, input_high0 = input_dwt[:b, ...], input_dwt[b:, ...]
        input_LL_dwt = self.dwt(input_LL)
        input_LL_LL, input_high1 = input_LL_dwt[:b, ...], input_LL_dwt[b:, ...]
        if self.wave_cond:
            wave_v = input_LL_dwt[b:b * 2]
            wave_h = input_LL_dwt[b * 2:b * 3]
            wave_d = input_LL_dwt[b * 3:b * 4]
            wave_cond = [wave_v, wave_h, wave_d]
            input_diff = [input_LL_LL] + wave_cond
        else:
            input_diff = [input_LL_LL]
        denoise_LL_LL = self.sample(input_diff, b)[-1]  # diff
        refine_in = self.idwt(torch.cat((self.clip1(denoise_LL_LL), input_high1), dim=0))
        refine_in = self.idwt(torch.cat((self.clip0(refine_in), input_high0), dim=0))  # 需要在初始化之前load refine
        if refine_hf:  # train+hf  and test
            with torch.no_grad():
                 pred_x = self.refine_model(refine_in)  # xxxxxxxxxxxxxxxx
        else:
            pred_x = refine_in
        pred_x  = pred_x
        pred_x = self.clipno(pred_x)
        pred_x = unnormalize_to_zero_to_one(pred_x)
        return pred_x.detach()
    def rgb_to_gray(self,t):
        # 输入 t: [B, 3, H, W]
        return 0.2989 * t[:, 0:1] + 0.5870 * t[:, 1:2] + 0.1140 * t[:, 2:3]  # [B, 1, H, W]
    def freeze(self):
        """
    ...
    def forward(self, img, refine_hf = True, pre_train =False ,*args, **kwargs):
        data_dict ={}
        if isinstance(img, list):
            b, c, h, w, device, img_size, = * \
                img[0].shape, img[0].device, self.image_size
        ...
        img = normalize_to_neg_one_to_one(img)
        # *******wavelet input ******************
        input_img = img[1] if self.training else img[0]
        input_dwt = self.dwt(input_img)
        input_LL, input_high0 = input_dwt[:b, ...], input_dwt[b:, ...]
        input_LL_dwt = self.dwt(input_LL)
        input_LL_LL, input_high1 = input_LL_dwt[:b, ...], input_LL_dwt[b:, ...]
        if self.wave_cond:
            ...
            input_diff = [input_LL_LL]
        if  refine_hf or not pre_train:  # ()
            denoise_LL_LL = self.sample(input_diff, b)[-1]       # diff
            refine_in = self.idwt(torch.cat((self.clip1(denoise_LL_LL), input_high1), dim=0))
            refine_in = self.idwt(torch.cat((self.clip0(refine_in), input_high0), dim=0))   # 需要在初始化之前load refine
        gt_img = img[0]
        # ****************gt wavelet and diff input***********
        gt_dwt = self.dwt(gt_img)
        gt_LL, gt_high0 = gt_dwt[:b, ...], gt_dwt[b:, ...]
        gt_LL_dwt = self.dwt(gt_LL)
        gt_LL_LL, gt_high1 = gt_LL_dwt[:b, ...], gt_LL_dwt[b:, ...]
        # 72，3，64，64
        img_diff = [gt_LL_LL] +input_diff
        data_dict["diff_loss"] = self.p_losses(img_diff, t, *args, **kwargs)
        # ****************************************************
        if not pre_train:  # train satge 2
            self.hook_outputs = []
            pred_x = self.refine_model(refine_in)
            multi_feats = self.hook_outputs
            data_dict["cls_loss"] = self.cls_loss(refine_in, feature =multi_feats,gt_labels = gt_labels)
            data_dict["photo_loss"], data_dict["frequency_loss"] = self.estimation_loss(pred_x, gt_img)
        return data_dict
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        if opts.phase == "train":
            self.dl_fog = cycle(self.accelerator.prepare(DataLoader(dataset[0], batch_size=8, shuffle=True, pin_memory=True, num_workers=4)))
            self.dl_light = cycle(self.accelerator.prepare(DataLoader(dataset[1], batch_size=4, shuffle=True, pin_memory=True, num_workers=2)))
            self.dl_rain = cycle(self.accelerator.prepare(DataLoader(dataset[2], batch_size=4, shuffle=True, pin_memory=True, num_workers=2)))
            self.dl_snow = cycle(self.accelerator.prepare(DataLoader(dataset[3], batch_size=4, shuffle=True, pin_memory=True, num_workers=2)))
            self.dl_blur = cycle(self.accelerator.prepare(DataLoader(dataset[4], batch_size=4, shuffle=True, pin_memory=True, num_workers=2)))
        else:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    if self.condition:
                        ...
                        raw_types = data['type_paths']
                        labels = torch.tensor([self.task2idx[t] for t in raw_types], dtype=torch.long,
                                              device=self.device)
                        data = [gt, cond_input, labels]
                    with self.accelerator.autocast():
                        if self.step >250000 :
                            ...
                            pre_train = True
                            refine_hf = False
                        output = self.model(data, refine_hf =refine_hf, pre_train=pre_train)
                        if not pre_train:
                            photo_loss = output["photo_loss"]
                            frequency_loss = output["frequency_loss"]
                            cls_loss = output["cls_loss"]
                            diff_loss = output["diff_loss"]
                            diff_loss = sum(diff_loss)
                            loss = photo_loss + frequency_loss + cls_loss +diff_loss
                        else:
                            diff_loss = output["diff_loss"]
                            diff_loss = sum(diff_loss)
                            loss = diff_loss
                ...
                if not pre_train:
                    pbar.set_description(
                        f'photo: {diff_loss.item():.4f},photo: {photo_loss.item():.4f}, freq: {frequency_loss.item():.4f}, cls: {cls_loss.item():.4f}'
                    )
                else:
                    pbar.set_description(
                        f'diff: {loss.item():.4f}')
    ...
    def test(self, sample=False, last=True, FID=False):
        ...
        if self.condition:
            ...
            for data in loader:
                torch.cuda.empty_cache()   # meiyou yong
                if self.condition:
                    file_ = data["A_paths"][0]
                ...
                with torch.no_grad():
                    ...
                    gt = data["gt"].to(self.device)
                    x_input_sample, pad_h, pad_w = pad_to_multiple(x_input_sample)
                    x_reconstructed = self.ema.ema_model.restore(
                        [x_input_sample], refine_hf = True)
                    x_reconstructed = x_reconstructed

                    x_reconstructed = remove_padding(x_reconstructed, pad_h, pad_w)
                    all_images_list = list(x_reconstructed)
                ...
                utils.save_image(all_images.cpu(), full_path, nrow=nrow)
            ...
            #calculate the metric
                ...
                del x_input_sample, gt, all_images, all_images_list,  x_reconstructed, data ,  opt_metric_,gt_img, sr_img
